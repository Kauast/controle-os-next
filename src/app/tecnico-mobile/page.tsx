...
import { ArrowLeft, BadgeCheck, Camera, CheckCircle2, LogOut, MapPin, Phone } from "lucide-react";
...
import { clearMobileToken, getMobileToken } from "@/lib/api/mobile-client";
...
export default function TecnicoMobilePage() {
  ...
  // Guarda de autenticação: sem token no localStorage → login
  useEffect(() => {
    if (!getMobileToken()) {
  ...
  if (!hydrated || !authed) {
    return (
      <div className="grid min-h-screen place-items-center bg-[#0d0d0d] text-[#a0a0a0]">
        Carregando...
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto min-h-screen max-w-md bg-panel pb-24">
      ...
      <header className="sticky top-0 z-30 flex items-center gap-3 border-b border-line bg-panel/90 px-4 py-3 backdrop-blur">
        <Link href="/tecnico-mobile/login/" className="text-muted" onClick={(e) => { e.preventDefault(); handleLogout(); }}>
          <ArrowLeft className="size-5" />
        </Link>
        <Image src="/logo.svg" alt="Logo" width={36} height={36} className="rounded-[10px]" />
        <div className="flex-1 leading-tight">
          <span className="text-[11px] text-muted">App do Tecnico</span>
        ...
        <Button variant="secondary" size="icon" onClick={handleLogout} title="Sair">
