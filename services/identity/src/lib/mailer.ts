...
export async function sendEmail(to: string, subject: string, html: string) {
  ...
  const from = process.env.SMTP_FROM ?? process.env.SMTP_USER ?? 'noreply@controle-os.com';

  if (!transport) {
    logger.info({ event: 'email_dev_mode', to, subject }, 'SMTP nao configurado — e-mail simulado em dev');
    return;
  }
...
export function buildPasswordResetEmail(name: string, resetUrl: string, domain: string): string {
  return `
